...
void CHandGrenade::WeaponIdle( void )
{
	...
	if ( m_flStartThrow )
	{
		...
		Vector vecThrow = (gpGlobals->v_forward * flVel + m_pPlayer->pev->velocity) * 2;
		...
		if ( !m_pPlayer->m_rgAmmo[ m_iPrimaryAmmoType ] )
		{
			...
			m_flTimeWeaponIdle = m_flNextSecondaryAttack = m_flNextPrimaryAttack = GetNextAttackDelay(0.05);// ensure that the animation can finish playing
		}
		return;
	}
