...
void CGauss::PrimaryAttack()
{
	...
	{
		PlayEmptySound( );
		m_flNextSecondaryAttack = m_flNextPrimaryAttack = GetNextAttackDelay(0);
		return;
	}
...
void CGauss::SecondaryAttack()
{
	...
	if ( m_pPlayer->pev->waterlevel == 3 )
	{
		...
		m_flNextSecondaryAttack = m_flNextPrimaryAttack = GetNextAttackDelay(0.07);
		return;
	}
	...
	if ( m_fInAttack == 0 )
	{
		if ( m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] <= 0 )
		{
			...
			m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.07;
			return;
		}
				...
				if ( !m_fPrimaryFire )
				{
					...
					if (!beam_tr.fAllSolid)
					{
						...
						if (n < flDamage)
						{
							...
							if ( g_pGameRules->IsMultiplayer() )
							{
								damage_radius = flDamage * 2.5;  // Old code == 2.5
							}
							else
							{
